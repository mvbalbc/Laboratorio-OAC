...
\author{
    ...
    \and
    \IEEEauthorblockN{Jo\~{a}o Gabriel Melo de Lima}
    \IEEEauthorblockA{241032617}
    \and
    \IEEEauthorblockN{Lucas Silva N\'{o}brega}
...
\begin{tabular}{>{\bfseries}p{0.25\linewidth} >{\centering\arraybackslash}p{0.61\linewidth}}
\toprule
...
\begin{itemize}
    \item Implementar os módulos principais do \textit{\ref{fig:datapath}} em Verilog (ULA, Unidade de Controle, Memórias, Geração de Imediatos, entre outros).
    ...
    \item Validar o funcionamento do processador por meio de simulações temporais, verificando a execução correta das instruções suportadas e a evolução dos principais sinais internos do \textit{\ref{fig:datapath}}.
    \item Relacionar a implementação em Verilog com o modelo teórico apresentado por Patterson e Hennessy, evidenciando como os componentes do \textit{\ref{fig:datapath}} e da unidade de controle cooperam durante a execução das instruções.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\linewidth]{datapath_novo.png}
    \caption{Fluxo de dados da Unidade Operativa Uniciclo RISC-V32I.}
    \label{fig:datapath}
\end{figure}

\subsection{Caminho de Dados (Datapath) e Arquitetura Uniciclo}
O caminho de dados (\textit{\ref{fig:datapath}}) é o conjunto de elementos funcionais que processam informações no processador, formado por elementos de estado e elementos combinacionais. Ele inclui componentes de armazenamento temporário, como o banco de registradores (RegFile) e o Contador de Programa (PC - Program Counter), além de componentes de transformação e roteamento, como a Unidade Lógica e Aritmética (ULA) e os multiplexadores. Na abordagem uniciclo, todas as etapas (busca, decodificação, execução, acesso à memória e escrita) são executadas no mesmo ciclo de clock, exigindo que o \textit{\ref{fig:datapath}} contenha recursos dedicados para cada etapa para evitar conflitos estruturais, como memórias separadas para dados e instruções (Arquitetura Harvard).

\subsection{Program Counter}
O Program Counter (PC) é o registrador responsável por armazenar o endereço da próxima instrução a ser buscada na memória de instruções. Comumente, o seu valor é incrementado em quatro bytes (PC+4) após cada instrução, pois todas as instruções do conjunto RV32I possuem tamanho fixo de 32 bits. Em instruções de desvio e salto, entretanto, o próximo valor do PC passa a ser determinado pelo resultado de cálculos envolvendo deslocamentos de imediatos ou de endereços calculados pela ULA.

\subsection{Multiplexadores}
Os multiplexadores são responsáveis por selecionar quais sinais percorrem o \ref{fig:datapath}. Eles permitem escolher, por exemplo, entre um operando vindo do banco de registradores ou do gerador de imediato, entre o resultado da ULA ou da memória para escrita nos registradores, e entre diferentes possibilidades para atualização do Program Counter.

\subsection{Arquitetura Harvard}
O processador desenvolvido utiliza arquitetura Harvard, caracterizada pela separação física entre memória de instruções e memória de dados. Desta forma, a busca de instruções ocorre independentemente dos acessos realizados à memória de dados, eliminando conflitos estruturais comuns em arquiteturas com memória unificada.

\subsection{Formatos de Instruções do RV32I}
O conjunto básico RV32I utiliza seis formatos principais de instrução: R, I, S, B, U e J. Cada formato organiza os campos opcode, registradores, imediato e códigos de função em posições específicas da palavra de 32 bits. Essa organização permite que a unidade de controle identifique rapidamente a operação a ser executada e que a unidade geradora de imediato reconstrua corretamente os valores utilizados durante a execução.


\subsection{Banco de Registradores}
O banco de registradores do RV32I contém 32 registradores de uso geral, cada um com 32 bits. Dois registradores podem ser lidos simultaneamente, enquanto apenas um pode ser escrito por ciclo de clock. O registrador x0 possui comportamento especial, permanecendo permanentemente com o valor zero independentemente de tentativas de escrita.

\subsection{Gerador de Imediatos}
Diversas instruções do conjunto RV32I utilizam valores imediatos codificados diretamente na instrução. Como esses valores encontram-se distribuídos em posições diferentes dependendo do formato (I, S, B, U ou J), torna-se necessária uma unidade responsável por reconstruir corretamente o imediato e realizar sua extensão de sinal para 32 bits.

\subsection{Execução de uma Instrução}
Qualquer tipo de operação e instrução, percorre cinco etapas lógicas:
\begin{enumerate}
\item Busca da instrução na memória;
\item Decodificação e leitura dos registradores;
\item Execução da operação na ULA;
\item Acesso à memória de dados, quando necessário;
\item Escrita do resultado no banco de registradores.
\end{enumerate}

Na arquitetura uniciclo essas etapas ocorrem durante um único período de clock.
...
\subsection{Sinais de Controle por Tipo de Opera\c{c}\~{a}o no RISC-V32I}
\begin{table}[h]
\centering
\label{tab:sinais_controle}
\setlength{\tabcolsep}{2pt}
\begin{tabular}{lcccccc}
\toprule
\textbf{Tipo de Opera\c{c}\~{a}o} & 
\rotatebox{50}{\textbf{Branch}} & 
\rotatebox{50}{\textbf{MemRead}} & 
\rotatebox{50}{\textbf{MemtoReg}} & 
\rotatebox{50}{\textbf{ALUOp}} & 
\rotatebox{50}{\textbf{MemWrite}} & 
\rotatebox{50}{\textbf{ALUSrc}} \\ 
\midrule
\textbf{Tipo-R}  & 0 & 0 & 0 & 10 & 0 & 0 \\
\textbf{Tipo-I}  & 0 & 0 & 0 & 11 & 0 & 1 \\
\textbf{Load}    & 0 & 1 & 1 & 00 & 0 & 1 \\
\textbf{Store}   & 0 & 0 & X & 00 & 1 & 1 \\
\textbf{Branch}  & 1 & 0 & X & 01 & 0 & 0 \\
\bottomrule
\end{tabular}
\end{table}
